...
class StatementOpportunityCalculator(BaseFactorCalculator):
    """
    Calculate statement opportunity factor based on ranking and perception gaps.
    
    Teams may be extra motivated when they have opportunities to make statements
    against higher-ranked opponents or prove they belong in elite conversations.
    """
    ...
    def __init__(self):
        ...
        self.weight = 0.10  # 10% of total (25% of situational context's 40%)
        self.category = "situational_context"
        self.description = "Statement opportunity and perception gap analysis"
        self._min_output = -1.5
        self._max_output = 1.5
        ...
        self.config = {
            ...
            'statement_multipliers': {
                'unranked_vs_top_5': 1.5,
                'unranked_vs_top_10': 1.2,
                'lower_ranked_vs_higher': 1.0,
                'upset_potential': 1.3
            },
            'motivation_factors': {
                'program_building': 0.4,
                'respect_earning': 0.6
            }
        }
    
    def calculate(self, home_team: str, away_team: str, context: Optional[Dict[str, Any]] = None) -> float:
        """Calculate statement opportunity factor."""
        if not context:
            return 0.0
        
        # Estimate team rankings/status (would need ranking data)
        home_status = self._estimate_team_status(context.get('home_team_data', {}))
        away_status = self._estimate_team_status(context.get('away_team_data', {}))
        
        # Calculate statement opportunities for each team
        home_opportunity = self._calculate_statement_opportunity(home_status, away_status, is_home=True)
        away_opportunity = self._calculate_statement_opportunity(away_status, home_status, is_home=False)
        
        # Net statement opportunity (positive favors home team)
        opportunity_diff = home_opportunity - away_opportunity
        
        return self.validate_output(opportunity_diff)
    
    def _estimate_team_status(self, team_data: Dict) -> Dict[str, Any]:
        """Estimate team's current status/ranking."""
        # This would ideally use real ranking data
        # For now, estimate based on record
        ...
        # Rough ranking estimation
        if win_pct >= 0.9:
            estimated_ranking = 5  # Top 5
        elif win_pct >= 0.8:
            estimated_ranking = 15  # Top 15
        elif win_pct >= 0.6:
            estimated_ranking = 35  # Ranked but lower
        else:
            estimated_ranking = 50  # Unranked
        
        return {
            'estimated_ranking': estimated_ranking,
            'win_percentage': win_pct,
            'is_top_25': estimated_ranking <= 25
        }
    
    def _calculate_statement_opportunity(self, team_status: Dict, opponent_status: Dict, is_home: bool) -> float:
        """Calculate statement opportunity for a team."""
        team_ranking = team_status.get('estimated_ranking', 50)
        opponent_ranking = opponent_status.get('estimated_ranking', 50)
        
        # No statement opportunity if opponent is clearly worse
        if team_ranking < opponent_ranking - 10:
            return 0.0
        
        # Big statement opportunity if facing much better opponent
        ranking_diff = team_ranking - opponent_ranking
        
        if ranking_diff > 20:  # Unranked vs top team
            return 1.0
        elif ranking_diff > 10:  # Lower ranked vs higher ranked
            return 0.6
        elif ranking_diff > 0:  # Slightly lower ranked
            return 0.3
        else:
            return 0.0  # Team is favored, no statement opportunity
    
    def get_output_range(self) -> Tuple[float, float]:
        """Get output range for statement opportunity factor."""
        return (self._min_output, self._max_output)
    
    def get_explanation(self, home_team: str, away_team: str, value: float, 
                       context: Optional[Dict[str, Any]] = None) -> Optional[str]:
        """Generate explanation for statement opportunity factor."""
        if abs(value) < 0.1:
            return "No significant statement opportunities identified"
        
        if value > 0:
            return f"{home_team} has a statement opportunity to prove themselves"
        else:
            return f"{away_team} has a statement opportunity to prove themselves"
    
    def get_required_data(self) -> Dict[str, bool]:
        """Statement opportunities work with basic team data."""
        return {
            'team_info': False,
            'coaching_data': False,
            'team_stats': False,
